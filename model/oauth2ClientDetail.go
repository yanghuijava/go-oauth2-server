...
type OauthClientDetail struct {
	...
	AccessTokenValidity   int       `gorm:"column:access_token_validity;NOT NULL"`                 // 设定客户端的access_token的有效时间值(单位:秒)，若不设定值则使用默认的有效时间值：2 * 60 * 60， 2小时
	RefreshTokenValidity  int       `gorm:"column:refresh_token_validity;NOT NULL"`                // 设定客户端的refresh_token的有效时间值(单位:秒)，若不设定值则使用默认的有效时间值：30 * 24 * 60 * 60 ，30天
